...
TestHK(test_hwnd, test_build_routine,test_destroy_routine) {
	global
	if !WinExist("ahk_id " test_hwnd) {
		GoSub, %test_build_routine%
		SetTimer, %test_destroy_routine%, off
	} else {
		GoSub, %test_destroy_routine%
	}
}
...
GUIRegularFont(guiname,color:=111111) {
	Gui, %guiname%: Font, c0%color% s11 w500 q5, Franklin Gothic Medium Cond
}
...
GuiBigPixelFont(guiname,color:=111111) {
	Gui, %guiname%: Font, c0%color% s15 w1950 q5, Terminal
}