...
;; GEN [General Environment Navigation] [ _ [modkey; x = none, a = alt, s = shift, c = ctrl] [action] _ ...]

GEN_xUp_aToTheTop_sEOL:
...
GEN_xLeft_aBack_sSelectToLeft_cSelectTLW:
if (getkeystate(modk_alt) == 1)
	sendInput !{left}
...
GEN_xRight_aForward_sSelectToRight_cSelectTRW:
if (getkeystate(modk_alt) == 1)
	sendInput !{right}
...
GEN_Home_mScrollLeft_mmfToTheTop:
	if (getkeystate(modk_alt) == 1) {
...
GEN_End_mScrollRight_mmfToTheBottom:
	if (getkeystate(modk_alt) == 1)
	{
		Lines := ScrollLines_1(HoverScrollMinLines, HoverScrollMaxLines, HoverScrollThreshold, HoverScrollCurve)
		HoverScroll(Lines, 0) ;0 = horizontal, 1 (or omit) = vertical
...
KDE_fClose:
	...
	if (Close_ID==CONTROL_hwnd)
		gosub CPDestroy
	
	else if (Close_ID==DIALOG_hwnd)
		gosub DialogDestroy
	
	else if (Close_ID==SETTINGS_hwnd)
		gosub SettingsDestroy
	
	else if (Close_ID==Overlay_id)
		return
	
	else if (Close_ID==LBrL_hwnd)
		return
...
KDE_fResize:
	...
	if (KDEm_Button = "U") {
		...
		if (WinTrans != KDE_winopacity_lock_opacity) {
			WinFade("ahk_id " KDEr_id,255,KDE_winfade_time_out,true,,true)
		}
...
ClearWindowLock() {
	...
	WinFade("ahk_id " KDEm_id,255,KDE_winfade_time_in,true,true)
	WinActivate
	i++
}
...
ApplyWindowLock(WinX, WinY, hwnd) {
	...
	WinFade("ahk_id " ct_hwnd,225,10,true,true)
...
WatchMouse:
	...
	if (LButtonState = "U")
	{
		...
		{
			; Transparency lock
			WinFade("ahk_id " KDEm_id,KDE_winopacity_lock_opacity,KDE_winopacity_lock_effect_time,false,,true)
		...
		if (KDEr_Button = "U") {
			WinGet, WinTrans, Transparent, ahk_id %KDEm_id%
			if (WinTrans <> KDE_winopacity_lock_opacity) {
				WinFade("ahk_id " KDEm_id,255,KDE_winfade_time_out,false,,true)
				RestoreCursors()
			}
		}
	...
	if (EscapeState == "D") {
		WinFade("ahk_id " KDEm_id,255,KDE_winfade_time_out,true,,true)
		RestoreCursors()
		SetTimer, WatchMouse, off
...
FEN_Up_mfToTheTop_mmsSelectToTop:
...
FEN_Down_mfToTheBottom_mmsSelectToBottom:
	if (getkeystate(modk_alt) == 1)
		sendInput {end}{space}
...
FEN_Left_mfDirUp_mmsSelectToLeft:
	if (getkeystate(modk_alt) == 1)
		SendInput, !{right}
...
FEN_Right_mfDirBack_mmsSelectToRight:
	if (getkeystate(modk_alt) == 1)
		sendInput !{left}
